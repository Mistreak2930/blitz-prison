-- Safe creation helpers for enums
DO $$ BEGIN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'post_reaction_type') THEN
...
-- 1) Create base tables first (profiles and user_roles)
...
-- user_roles table before has_role()
...
create policy if not exists "Anyone authenticated can read user roles"
...
create policy if not exists "Announcements are viewable by everyone"
  on public.announcements for select using (true);

create policy if not exists "Admins or announcement managers can insert"
  on public.announcements for insert to authenticated
  with check (public.has_role(auth.uid(),'admin') or public.has_role(auth.uid(),'announcements_manager'));

create policy if not exists "Admins or announcement managers can update"
...
create policy if not exists "Admins or announcement managers can delete"
...
create policy if not exists "News are viewable by everyone"
  on public.news for select using (true);

create policy if not exists "Admins or news updaters can insert"
...
create policy if not exists "Admins or news updaters can update"
...
create policy if not exists "Admins or news updaters can delete"
...
create trigger if not exists update_forum_posts_updated_at
before update on public.forum_posts
for each row execute function public.update_updated_at_column();

create policy if not exists "Forum posts are viewable by everyone"
  on public.forum_posts for select using (true);

create policy if not exists "Users can create their own forum posts"
  on public.forum_posts for insert to authenticated
  with check (auth.uid() = user_id);

create policy if not exists "Owners or staff can update forum posts"
...
create policy if not exists "Owners or staff can delete forum posts"
...
create policy if not exists "Senders can create messages"
  on public.private_messages for insert to authenticated
  with check (auth.uid() = sender_id);

create policy if not exists "Recipients can mark messages"
...
create policy if not exists "Staff can view moderation logs"
  on public.moderation_logs for select to authenticated
  using (public.has_role(auth.uid(),'admin') or public.has_role(auth.uid(),'moderator'));

create policy if not exists "Staff can insert moderation logs"
...
-- Chat tables first, then is_participant()
...
-- is_participant function now that table exists
...
create policy if not exists "Participants can view participants"
  on public.chat_participants for select to authenticated
  using (public.is_participant(conversation_id, auth.uid()));

create policy if not exists "Creators or users can add participants"
...
create policy if not exists "Users can remove themselves or creators can manage"
...
create policy if not exists "Senders can update their messages"
...
create policy if not exists "Participants can view message reactions"
...
create policy if not exists "Users can react to messages"
...
create policy if not exists "Users can remove their message reactions"
...
create trigger if not exists update_chat_requests_updated_at
...
create policy if not exists "Sender or recipient can view chat requests"
  on public.chat_requests for select to authenticated
  using (auth.uid() = sender_id or auth.uid() = recipient_id);

create policy if not exists "Senders can create chat requests"
  on public.chat_requests for insert to authenticated
  with check (auth.uid() = sender_id);

create policy if not exists "Recipients can update chat requests"
